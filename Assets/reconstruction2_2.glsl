...
const int freedom = 10;

layout (local_size_x = 10, local_size_y = 10, local_size_z = 9) in;
...
shared float integrals[10*10*9];
...
void main()
{
  ...
  uint threadXY = threadX * 10 + threadY;
  ...
  barrier();
    ...
    points = calculateControlPoints(vec3[continuity+1](inBuf.data[2*id].p.xyz, d0, dd0), vec3[continuity+1](inBuf.data[2*id+1].p.xyz, d1, dd1), extras);
