...
layout (local_size_x = 6, local_size_y = 6, local_size_z = 9) in;
...
void main()
{
  ...
  uint threadXY = threadX * 6 + threadY;
  ...
  barrier();
    ...
    points = calculateControlPoints(vec3[continuity+1](inBuf.data[2*id].p.xyz, d0, dd0, ddd0), vec3[continuity+1](inBuf.data[2*id+1].p.xyz, d1, dd1, ddd1));
    if(infnorm)
